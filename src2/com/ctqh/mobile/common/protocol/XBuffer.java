...
//		}
	...
	public int forward(int count) throws IOException {
		int idx = this.index;
	...
	public int end_batch(int batchCount) {
		int is_big=1;
	...
	public void writeString(int pos,String str){
		byte[] bytes=str.getBytes(PROTOCOL_CHARSET);
		writeshort(pos,(short)bytes.length);
	...
	public static long read(byte[] input, int idx, int len) {
	...
	public static void write(byte[] output, int idx,int len, long value){
		for (int i = 0; i < len; i++) {

...
/**
 * 传输类型无关的句柄，表示了两点间的连接。
 * 提供了用户定义的属性。用户属性石附加在会话上的应用程序数据，可以表示高级协议的状态也可以在过滤器和处理器之间交换数据。
 * 可以向下转型到合适类型。
 * 线程安全，同时也请保证IoFilter的实现是线程安全的。
 ...
 * It often contains objects that represents the state of a higher-level protocol and 
 ...
 * But please note that performing more than one {@link #write(Object)} calls at the same time will cause the {@link IoFilter#filterWrite(IoFilter.NextFilter,IoSession,WriteRequest)} to be executed simultaneously, 
 * and therefore you have to make sure the {@link IoFilter} implementations you're using are thread-safe, too.
 *
 * </p>
 * <p/>
